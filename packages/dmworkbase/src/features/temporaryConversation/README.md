...
- Only message-free virtual rows use the avatar/name-only appearance. Real conversations keep their unread/mention indicators, message preview, reminders, and timestamp while temporarily placed, including incoming messages after switching to another chat and before any full list refresh. Row unread indicators use the same data as the aggregate badges and remain visible when unread navigation highlights them. Existing rows are deduplicated without mutating conversation data. Once a virtual entry resolves to a real conversation, the regular row displays its full metadata immediately.
...
- Check an existing unpinned temporary row receiving an incoming mention after switching away: its visible unread agrees with aggregate counting, its preview and timestamp remain visible, and unread navigation highlights its rendered badge without requiring a full refresh.
