...
// 只 mock modalConfirm（捕获 content 元素）与 Semi（TextArea/Toast 桩），i18n 走真实实现。
...
const { modalConfirmMock } = vi.hoisted(() => ({ modalConfirmMock: vi.fn() }));
...
describe("denyReasonDialog interpolation (real t)", () => {
  it("substitutes interpolation vars instead of rendering literal {{...}}", () => {
    ...
    const config = modalConfirmMock.mock.calls[0][0];
