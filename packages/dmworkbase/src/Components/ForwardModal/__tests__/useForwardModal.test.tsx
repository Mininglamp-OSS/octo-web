 ...
 *
 * 数据源：列表主体复用本地会话 + group/my + 好友装配（零权限回归）；「关注」Tab 走
 * SidebarService follow 同步，「最近」Tab 走 SidebarService recent 同步（后端权威
 * 最近集合，按 timestamp 降序），二者均与 ChatSelectorModal 对齐。这里守护：Tab
 * 作用域过滤、群→子区嵌套、关注/最近集合过滤与排序、以及授权在任意 Tab 下确认仍生效。
 */
describe("useForwardModal — four-Tab selector (issue #661)", () => {
    ...
    it("skips the follow sync when deviceId is empty (doomed request guard) → followed tab empty", async () => {
        hoisted.deviceId = ""
        hoisted.conversations = [makeConv("g1", CT_GROUP, "Group 1", { timestamp: 100 })]
