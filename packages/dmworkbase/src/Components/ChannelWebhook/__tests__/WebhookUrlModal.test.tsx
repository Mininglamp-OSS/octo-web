...
describe('WebhookUrlModal renderExample branch mapping', () => {
  ...
  it('native row also renders a second curl demonstrating mention (uids + render)', async () => {
    await render();
    const pres = Array.from(
      container.querySelectorAll<HTMLPreElement>('pre.wk-webhook-url__example-code')
    );
    // 基础示例 + @提及示例：应存在一条含 "mention" 的 native curl。
    const mentionPre = pres.find((p) => /"mention"/.test(p.textContent || ''));
    expect(mentionPre).toBeTruthy();
    expect(mentionPre!.textContent).toContain('"render"');
    expect(mentionPre!.textContent).toContain('"uids"');
    // mention 示例仍是 native 的 content 结构，绝不能误用 wecom 的 msgtype。
    expect(mentionPre!.textContent).not.toContain('msgtype');
  });
});
