...
import { voiceSettingsStore, type VoiceSettings } from "../../Service/VoiceSettingsStore";
...
export default function SettingsCenter({ visible, isDesktop = false, environment, accountCenterUrl, onClose, onLogout, onSecretsClosed, onAbout, onChangelog, onOpenOnboarding, openSecretsRequest }: SettingsCenterProps) {
  ...
  const [voiceSettings, setVoiceSettings] = useState<VoiceSettings>(() => voiceSettingsStore.get());
  React.useEffect(() => voiceSettingsStore.subscribe(setVoiceSettings), []);
  const voiceShortcutConfigured = voiceSettings.enabled && voiceSettings.shortcutEnabled;
  const availableGroups = useMemo(
    () => getAvailableSettingsGroups({ environment: runtimeEnvironment }).map((group) => ({
      ...group,
      items: group.items.filter((item) => item.id !== "shortcuts" || voiceShortcutConfigured),
    })).filter((group) => group.items.length > 0),
    [runtimeEnvironment, voiceShortcutConfigured],
  );
  ...
  React.useEffect(() => {
    if (!availableGroups.some((group) => group.items.some((item) => item.id === selectedId))) {
      setSelectedId(availableGroups[0]?.items[0]?.id ?? "general");
    }
  }, [availableGroups, selectedId]);
