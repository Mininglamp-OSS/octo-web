...
export default function TemplateSelectorModal({
    ...
    const saveEditor = async () => {
        ...
        try {
            if (creatingCustomTemplate) {
                ...
                const updated = await dataSource.updateCustom(
                    editingTemplate.id,
                    {
                        label,
                        description,
                ...
                const updated = await dataSource.updateBuiltIn(
                    editingTemplate.id,
                    {
                        label,
                        description,
    ...
    const resetBuiltIn = async () => {
        if (!editingTemplate || editingTemplate.is_custom || mutationBusy)
            return;
        setMutationBusy(true);
        setMutationError("");
        try {
    ...
    const deleteCustom = async () => {
        ...
        try {
            ...
            setTemplates((current: TopicTemplate[]) =>
                current.filter(
                    (template: TopicTemplate) => template.id !== target.id
                )
            );
        ...
        <>
                    ...
                    <div className="wk-template-selector__body">
                        {builtInTemplates.length > 0 ? (
                            <section className="wk-template-selector__section">
                                ...
                                <div
                                    className="wk-template-selector__grid"
                                    role="list"
                                >
                        ...
                        <section className="wk-template-selector__section">
                            ...
                            {!canCreateCustomTemplate && (
                                <p className="wk-template-selector__hint">
                                    {labels.limitReached}
                                </p>
                            )}
            ...
            >
                <div className="wk-template-selector-editor__field">
                    <label htmlFor="wk-template-selector-name">
                        {labels.nameLabel}
                    </label>
                    <input
                ...
                <p className="wk-template-selector-editor__hint">
                    {labels.editHint}
                </p>
                {mutationError && (
                    <p
                        className="wk-template-selector-editor__error"
                        role="alert"
                    >
                        {mutationError}
                    </p>
