...
import {
  ...
  type SummaryWorkspaceDocumentDTO,
...
function resolveHistoryResultType(
  ...
  const resultType =
    artifactResultType ?? previewResultType ?? messageResultType;
  if (
  ...
  const messages = history.messages.map((message) => {
    ...
    const card: SummaryWorkbenchCardView | undefined = preview
      ...
      ? {
          ...
          isStale:
            pendingProposal.scope_version !== authoritativeState.scopeVersion,
          actions: [...pendingProposal.available_actions],
        }
    ...
    const workbenchMessage: SummaryWorkbenchMessage = {
      ...
      availableActions: artifact?.actions ?? [
...
function decodeContext(
  ...
  const documents = requireArray(
    record.documents ?? [],
    `${path}.documents`
  ).map((document, index): SummaryWorkspaceDocumentDTO => {
    const documentPath = `${path}.documents[${index}]`;
    const documentRecord = requireRecord(document, documentPath);
    return {
      document_id: requireString(
        documentRecord.document_id,
        `${documentPath}.document_id`
      ),
      title: optionalString(documentRecord.title, `${documentPath}.title`),
    };
  });
  const template =
