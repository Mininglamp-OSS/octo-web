...
export async function createAgentSummary(
    ...
    const resp = idempotencyKey
        ? await summaryAxios.post(`${BASE}/summaries/agent`, params, {
...
function isFinalizeTaskStatus(status: unknown): status is TaskStatusType {
...
/**
 * POST /summaries/agent/finalize —— 异步定稿。返回 202 的 { task_id, status:<TaskStatus> }。
...
export async function finalizeAgentSummary(
    params: CreateAgentSummaryParams,
    idempotencyKey: string,
): Promise<{ task_id: number; status: TaskStatusType }> {
    ...
    const data = resp.data?.data as { task_id?: unknown; status?: unknown } | undefined;
    if (!data || typeof data.task_id !== 'number' || data.task_id <= 0) {
    ...
    if (!isFinalizeTaskStatus(data.status)) {
...
export async function saveAgentSummaryViaFinalize(
    ...
    try {
        ...
        // The legacy route supports the same header. Reuse the logical-save key
        // so a lost legacy response can replay the original task instead of
        // creating a duplicate on the user's next click.
        const legacy = await createAgentSummary(params, trackProps, options.idempotencyKey);
