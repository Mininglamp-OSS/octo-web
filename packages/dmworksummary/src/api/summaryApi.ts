...
function extractErrorMessage(err: unknown): string {
  ...
  if (status === 404 && raw.toLowerCase().includes("404")) {
    raw =
      "Summary refine API is not available. Please restart octo-smart-summary with the latest branch.";
  }
  if (
    status === 503 &&
    raw.toLowerCase().includes("refine service is not configured")
  ) {
    raw =
      "Summary refine service is not configured. Please enable LLM config for summary-api.";
  }
...
export async function createAgentSummary(
  ...
  if (envelopeCode !== 0) {
    ...
    const err = new Error(
      resp.data?.message || "create agent summary failed"
    ) as Error & {
  ...
  if (!data || typeof data.task_id !== "number" || data.task_id <= 0) {
    ...
    throw new Error(
      resp.data?.message || "create agent summary returned no task_id"
    );
  }
...
export interface SummaryWorkspaceIdempotentOptions
  extends SummaryWorkspaceRequestOptions {
  idempotencyKey: string;
}

/** Raw transport for the new summary workspace. Runtime DTO validation lives in the Service adapter. */
export async function getSummaryWorkspaceCapabilities(
...
async function requestSummaryWorkspace(
  ...
  try {
    ...
    return unwrapSummaryWorkspaceEnvelope(
      response.data,
    ...
    const responseData = response
      ? readRecordProperty(response, "data")
      : undefined;
    const httpStatus = response
      ? readNumberProperty(response, "status")
      : undefined;
    const businessError = createSummaryWorkspaceBusinessError(
      responseData,
      httpStatus
    );
    if (businessError) throw businessError;
    const errorSource = responseData
      ? readRecordProperty(responseData, "error") ?? responseData
      : undefined;
    const effectiveHttpStatus =
      readNumberProperty(errorSource, "http_status") ?? httpStatus;
    const message =
    ...
    throw new SummaryWorkspaceApiError({
      ...
      detail:
        readStringProperty(errorSource, "detail") ??
        readStringProperty(errorSource, "details"),
      retryable:
        effectiveHttpStatus === undefined || effectiveHttpStatus >= 500,
    });
  }
}

function unwrapSummaryWorkspaceEnvelope(
  payload: unknown,
  allowNullData = false
): unknown {
...
function createSummaryWorkspaceBusinessError(
  ...
  const effectiveHttpStatus =
    readNumberProperty(source, "http_status") ?? httpStatus;
  if (effectiveHttpStatus !== undefined && effectiveHttpStatus >= 500)
    return undefined;
  const data = readRecordProperty(payload, "data");
  const retryable = isRetryableSummaryWorkspaceCode(code);
  return new SummaryWorkspaceApiError({
    ...
    httpStatus: effectiveHttpStatus,
    detail:
      readStringProperty(source, "detail") ??
      readStringProperty(source, "details"),
    recoveryAction: readStringProperty(data, "recovery_action"),
    taskId: readSummaryWorkspaceTaskId(data),
    retryable,
  });
...
function readSummaryWorkspaceTaskId(
  data: Record<string, unknown> | undefined
): number | undefined {
  return (
    readNumberProperty(data, "task_id") ??
    readNumberProperty(data, "existing_task_id")
  );
}
        ...
        for (const line of lines) {
          if (line.startsWith("event:")) {
            ...
            if (pendingEvent && pendingData) {
              const dispatched = parseAndDispatch(
                pendingEvent,
                pendingData,
                handlers
              );
...
function decodeAgentStreamHttpError(
  status: number,
  body: string
): AgentErrorEvent {
  let payload: unknown;
  ...
  const nestedError = envelope
    ? readRecordProperty(envelope, "error")
    : undefined;
...
export async function getSummaryShare(
  shareId: string,
  spaceId?: string
): Promise<GetSummaryShareResponse> {
  return get(
...
export async function regeneratePersonalSummary(
  ...
  return post(
    `/summaries/${taskId}/personal-regenerate`,
    body,
    "smart_summary_regenerated"
  );
}
...
export async function refinePersonalSummary(
  ...
  try {
    const resp = await summaryAxios.post(
      `${BASE}/summaries/${taskId}/personal-refine`,
      body,
      { timeout: 95000 }
    );
    return resp.data?.data ?? resp.data;
...
export async function getMembers(taskId: number): Promise<MemberStatus[]> {
  const data = await get<{ members: MemberStatus[] }>(
...
export async function resetMyTopicTemplate(
  templateId: string
): Promise<TopicTemplate> {
  const data = await del<{ template: TopicTemplate }>(
    `/summary-templates/${encodeURIComponent(templateId)}/my`
  );
  return data.template;
}
...
function normalizeScheduleItem<
  T extends { is_active?: unknown } | null | undefined
>(item: T): T {
...
export async function getSchedule(scheduleId: number): Promise<ScheduleItem> {
  return normalizeScheduleItem(
    await get<ScheduleItem>(`/summary-schedules/${scheduleId}`)
  );
}
...
export async function updateSchedule(
  ...
  return normalizeScheduleItem(
    await put<ScheduleItem>(
      `/summary-schedules/${scheduleId}`,
      params,
      "smart_summary_timer_configured"
    )
  );
...
export async function getChatCandidates(params?: {
  ...
  const data = await get<ChatCandidate[]>(
    "/summary-chat-candidates",
...
export async function getMemberCandidates(params?: {
  ...
  const data = await get<MemberCandidate[]>(
    "/summary-member-candidates",
    params as Record<string, unknown>
  );
  return data || [];
}
 ...
 * 隐藏入口，正常路径上不会走到这个分支。
 */
export async function convertSummaryToDoc(
  title: string,
  markdown: string
