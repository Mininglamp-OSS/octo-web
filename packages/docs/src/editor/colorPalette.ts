...
/** Fraction each text swatch is mixed toward white for use as a highlight background. */
export const HIGHLIGHT_TINT = 0.8

/** Derive a light background from a canonical `#rrggbb` swatch without changing its hue family. */
export function toHighlightTint(hex: string, amount: number = HIGHLIGHT_TINT): string {
  const match = /^#([0-9a-fA-F]{6})$/.exec(hex)
  if (!match) throw new Error(`toHighlightTint expects #rrggbb, got: ${hex}`)
  const value = Number.parseInt(match[1], 16)
  const lift = (channel: number) => Math.round(channel + (255 - channel) * amount)
  const channel = (shift: number) => lift((value >> shift) & 0xff).toString(16).padStart(2, '0')
  return `#${channel(16)}${channel(8)}${channel(0)}`
}

/**
 * Highlights retain the canonical 5×9 preset order, but use light tints rather than foreground
 * inks. This keeps dark document text legible even for the canonical black/deep-colour swatches.
 */
export const HIGHLIGHT_COLORS = TEXT_COLORS.map((color) => toHighlightTint(color)) as readonly string[]
