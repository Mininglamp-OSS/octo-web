 ...
 * pick = one caller transaction, matching the sheet.
 *
...
export function ColorSplitControl({
  ...
  // Univer 0.25 updates only its internal HSV when a preset is clicked; its public onChange fires
  // for the spectrum/custom-confirm path, not for a preset. Resolve the clicked preset by its stable
  // row/button order and commit it here so white and every other swatch applies in the same click.
