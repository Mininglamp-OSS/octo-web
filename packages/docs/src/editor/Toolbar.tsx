...
import { FONT_FAMILIES, isFontFamilySupported } from './fontFamilies.ts'
          ...
          <Btn
            label={
              <span className="octo-math-color-label">
                <FontColorDoubleIcon className="octo-tb-icon" extend={{ colorChannel1: (editor.getAttributes(mathNodeName() ?? 'inlineMath').color as string) || DEFAULT_TEXT_COLOR }} />
          ...
          {colorOpen && (
            <ConfigProvider mountContainer={getUniverPortal()} locale={designLocale()}>
              <span className="octo-color-popover octo-color-popover--picker octo-theme">
                <ColorPicker
                  format="hex"
                  value={(editor.getAttributes(mathNodeName() ?? 'inlineMath').color as string) || DEFAULT_TEXT_COLOR}
...
// Highlight and font colour both render the canonical 5×9 Univer Sheet palette through the shared
// ColorSplitControl. The first preset is opaque white; clearing is a separate explicit action.
...
/** Font-size presets used by the toolbar dropdown live in the shared module below (SCHEMA_VERSION 7).
 *  Extracted so the whiteboard text toolbar reuses the same size steps. */
...
function FontSizeSelect({ editor }: { editor: Editor }) {
  ...
  return (
    ...
    <span className="octo-tb-sel octo-tb-sel--size">
      <Select
        className="octo-tb-sheet-select"
        borderless
        value={current}
        options={options}
        onChange={(v) => {
...
export function FontFamilySelect({ editor }: { editor: Editor }) {
  ...
  // Probe local font availability once per mount via the shared catalogue probe (mirrors Univer
  // IFontService.isFontSupported; never downloads a face) so the docs picker flags a missing font
  // exactly like the Board does. (The Sheet keeps Univer's own probe — see fontFamilies.ts.)
  const fontSupport = useMemo(
    () => new Map(FONT_FAMILIES.map((f) => [f.value, isFontFamilySupported(f.value)])),
    [],
  )
  ...
  // like the sheet's picker. An unavailable font gets a localized "using a substitute" warning marker.
  //
  // The marker is deliberately omitted from the CURRENTLY-APPLIED font's label: Univer's Select reuses
  // the selected option's label node as the collapsed trigger's content (its `displayValue`), so
  // embedding the marker there would park an info icon at the right inner edge of the 172px trigger —
  // a visual dead zone over the primary control (P1-1). Keeping the current option text-only leaves the
  // trigger clean while every OTHER unavailable row still shows the marker when the menu is open. The
  // marker also no longer stops pointer/click propagation: picking an unavailable font is a legitimate
  // action, so a click on the icon should fall through to the option's own selection.
  const options = [
    { label: t('docs.toolbar.font.arial'), value: '' },
    ...FONT_FAMILIES.map((f) => ({
      label: (
        <span className="octo-tb-font-opt" style={{ fontFamily: f.value }}>
          <span>{t(f.labelKey)}</span>
          {fontSupport.get(f.value) === false && f.value !== current && (
            <Tooltip
              title={t('docs.toolbar.fontNotSupported')}
              placement="right"
              asChild
            >
              <span
                className="octo-tb-font-warning"
                role="img"
                aria-label={t('docs.toolbar.fontNotSupported')}
              >
                <InfoIcon />
              </span>
            </Tooltip>
