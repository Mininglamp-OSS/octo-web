...
function control(initialColor = '#e03131', portalPopover = false, commitPresetOnPointerDown = false) {
  const main = vi.fn()
  const picked = vi.fn()
  const view = render(
    <ColorSplitControl
      ...
      commitPresetOnPointerDown={commitPresetOnPointerDown}
    />,
  )
...
describe('ColorSplitControl', () => {
  it('commits the opaque white preset on pointerdown before Excalidraw can tear down the picker', () => {
    const view = control('#e03131', false, true)
  ...
  it('leaves docs preset commits to Univer semantic onChange', () => {
    const view = control()
    fireEvent.click(view.container.querySelector('.octo-color-caret-btn')!)
    fireEvent.pointerDown(view.getByTestId('preset-white'))
    expect(view.picked).not.toHaveBeenCalled()
    expect(view.container.querySelector('.octo-color-popover')).not.toBeNull()
  })
