...
interface Props {
  ...
  /**
   * Commit an explicit user-picked colour to the shared collab doc (canvas background sync contract authority gate). Called
...
export function BoardCanvasColorControl({ excalidrawAPI, readOnly = false, rootRef, onColorCommit }: Props) {
  ...
  useEffect(() => {
    ...
    const bindWhenReady = () => {
      if (disposed) return
      const next = rootRef?.current ?? null
      if (next) {
        root = next
        root.addEventListener(BOARD_CANVAS_COLOR_EVENT, onRequest)
      } else {
        frame = requestAnimationFrame(bindWhenReady)
      }
    }
    bindWhenReady()
    return () => {
      ...
      root?.removeEventListener(BOARD_CANVAS_COLOR_EVENT, onRequest)
    }
  }, [readOnly, rootRef])
  ...
  const apply = (color: string) => {
    ...
    excalidrawAPI!.updateScene({
      appState: { viewBackgroundColor: color } as never,
