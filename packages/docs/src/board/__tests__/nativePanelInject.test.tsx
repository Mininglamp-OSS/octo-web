 ...
 *     size calls `excalidrawAPI.updateScene` with the mutated element carrying a fresh CAS stamp
 *     (a subtree assertion that boardElementBump ran).
...
describe('installNativePanelExtensions', () => {
  ...
  it('exposes only left, center, and right alignment', () => {
    const t1: El = {
  ...
  it('uses attribute-targeted controls for a mixed text/shape/frame selection', () => {
    const text: El = {
  ...
  it.each(['rectangle', 'diamond', 'ellipse', 'arrow', 'line', 'freedraw', 'embeddable'])(
    ...
    (activeTool) => {
      ...
      expect(root.querySelectorAll('.octo-color-split')).toHaveLength(
        ['rectangle', 'diamond', 'ellipse', 'embeddable'].includes(activeTool) ? 2 : 1,
      )
      dispose()
    },
  )
  ...
  it('clamps stale panel scroll after an alignment write reflows the injected controls', async () => {
    ...
    Object.defineProperty(panel, 'scrollHeight', { configurable: true, value: 260 })
    panel.scrollTop = 180
    act(() => {
