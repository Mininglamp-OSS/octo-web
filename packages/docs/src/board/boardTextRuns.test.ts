...
describe('boardTextRuns', () => {
  ...
  it('uses textarea-compatible UTF-16 offsets for emoji', () => {
    ...
    expect(transformTextRuns([{ start: 3, end: 4, italic: true }], text, 'A😀xB', undefined, 4)).toEqual([
      { start: 4, end: 5, italic: true },
    ])
  })

  it('uses the caret to locate insertion among repeated neighboring characters', () => {
    expect(
      transformTextRuns([{ start: 0, end: 2, bold: true }], 'aaaa', 'aaaaa', undefined, 1),
    ).toEqual([{ start: 1, end: 3, bold: true }])
  })

  it('uses the caret to locate deletion among repeated neighboring characters', () => {
    expect(
      transformTextRuns([{ start: 0, end: 2, bold: true }], 'aaaa', 'aaa', undefined, 0),
    ).toEqual([{ start: 0, end: 1, bold: true }])
  })
