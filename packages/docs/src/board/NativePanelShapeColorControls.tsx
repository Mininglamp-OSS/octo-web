...
export function NativePanelShapeColorControls({
  ...
  const selected = snapshot.elements.filter((el) => !el.isDeleted && snapshot.selectedIds[el.id])
  ...
  const shared = (key: 'strokeColor' | 'backgroundColor') => {
    ...
    const values = new Set(selected.map((el) => el[key]).filter((value): value is string => !!value))
  ...
  const apply = (key: 'strokeColor' | 'backgroundColor', color: string) => {
    ...
    const elements = live.map((el) => (ids.has(el.id)
      ...
      : el))
    excalidrawAPI.updateScene({ elements: elements as never })
  }
        ...
        <ConfigProvider mountContainer={getUniverPortal()} locale={designLocale()}>
          ...
          >
            <ColorSplitControl
              ...
              onMainClick={(value) => apply(key, value)}
  ...
  return (
    ...
    >
      ...
      {hasBackground && control('backgroundColor', t('docs.board.nativePanel.backgroundColor'), DEFAULT_HIGHLIGHT_COLOR)}
    </div>
  )
}
