 ...
 * These three controls disagree with the docs table toolbar: docs offers 12/14/16/18/24/32 sizes,
 * a 10-colour text palette, and four alignments including justify — plus bold / italic / underline,
 * which Excalidraw has no equivalents for at all.
 *
 ...
 * indistinguishable from a native section. Every scene mutation goes through `bumpElementStamp`
 * (see ./boardElementBump.ts) so the collab CAS gate propagates the write.
 *
...
const FONT_FAMILY_BUTTON_TESTIDS = ['font-family-octo-2001', 'font-family-hand-drawn', 'font-family-normal', 'font-family-code', 'font-family-show-fonts'] as const
...
const SHAPE_BACKGROUND_TYPES = new Set(['rectangle', 'diamond', 'ellipse', 'embeddable'])
...
function readTextSelection(api: ExcalidrawImperativeAPI): TextSelectionSnapshot | null {
  ...
  const selected = all.filter((el) => !el.isDeleted && activeSet.has(el.id))
  if (selected.length === 0) return null
  // Text-only guard: any non-text element in the selection means we keep the upstream picker
  // exactly as-is (a mixed selection is a shape+text edit and should behave like Excalidraw).
  if (selected.some((el) => el.type !== 'text')) return null
  return { elements: all, selectedIds, toolDefault: false }
}
...
function readShapeSelection(api: ExcalidrawImperativeAPI): ShapeSelectionSnapshot | null {
  ...
  if (selectedIds.length === 0) {
    ...
    return SHAPE_COLOR_TYPES.has(type)
      ? { selectedIds: [], toolDefault: true, hasBackground: SHAPE_BACKGROUND_TYPES.has(type) }
      : null
  }

  const active = new Set(selectedIds)
  const selected = all.filter((el) => !el.isDeleted && active.has(el.id))
  if (selected.length === 0 || selected.some((el) => !SHAPE_COLOR_TYPES.has(el.type))) return null
  return {
    selectedIds,
    toolDefault: false,
    hasBackground: selected.every((el) => SHAPE_BACKGROUND_TYPES.has(el.type)),
  }
}
...
export function installNativePanelExtensions(
  ...
  function renderShapeInto(panelColumn: HTMLElement, snapshot: ShapeSelectionSnapshot): void {
    ...
    let host = panelColumn.querySelector<HTMLElement>(`:scope > .${HOST_CLASS}`)
    const renderKey = `shape:${snapshot.toolDefault ? 'defaults' : [...snapshot.selectedIds].sort().join(',')}:${snapshot.hasBackground}`
    const controls = (
  ...
  function sweep(): void {
    ...
    for (const panelColumn of panelColumns) {
      const textSelection = readTextSelection(api)
      const shapeSelection = textSelection ? null : readShapeSelection(api)
  ...
  const selectionSignature = (): string => {
    const text = readTextSelection(api)
    if (text) return `t:${text.toolDefault ? 'd' : 's'}:${[...text.selectedIds].sort().join(',')}`
    const shape = readShapeSelection(api)
    if (shape) {
      return `s:${shape.toolDefault ? 'd' : [...shape.selectedIds].sort().join(',')}:${shape.hasBackground}`
    }
    return 'none'
  }
