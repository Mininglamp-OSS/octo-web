/**
 * NativePanelTextControls — the React tree rendered INSIDE Excalidraw's own properties panel
 * (`.panelColumn`) by installNativePanelExtensions. Groups every text-related control the docs
 * table toolbar exposes so the whiteboard's native right panel matches the docs surface exactly:
 *
 *   - Font size          — 12 / 14 / 16 / 18 / 24 / 32 (shared FONT_SIZES with the docs toolbar)
 *   - Text color         — the shared {@link ColorSplitControl} (preset swatch grid + custom
 *                          spectrum + hex "more colours" flow), identical to the docs table toolbar
 *   - Bold / Italic / Underline — element-level toggles stored under `customData.*` because
 *                          Excalidraw's text model has no character-level marks
 *   - Horizontal align   — left / center / right
 *
 * Every mutation goes through `bumpElementStamp` so the collab CAS gate propagates the write.
 * State is derived directly from the imperative Excalidraw API via `useSyncExternalStore`; the
 * store subscribes to `onChange` and reads scene + selection on every getSnapshot. Multi-select
 * with divergent values shows a "mixed" sentinel that cannot be re-selected once the user has
 * committed a real value.
 *
 * This module owns none of the observer / injection lifecycle — that lives in ./nativePanelInject.tsx.
 */
...
import { TEXT_COLORS } from '../editor/colorPalette.ts'
...
/** Product default ink. Clearing text colour must match newly-created Board text. */
...
/** Board's font-availability check. Delegates to the ONE authoritative probe shared with the Doc
 * toolbar (and mirroring Univer IFontService.isFontSupported), so all three surfaces flag a missing
 * local font identically. Retained as a thin re-export for the existing call site below. */
...
export function NativePanelTextControls({ excalidrawAPI, initialSelection, toolDefault = false, defaultsKey, runtime }: Props) {
  ...
  const snapshot = useSyncExternalStore<Snapshot>(
    ...
    () => {
      ...
      if (editing && editing.type === 'text' && selectedMap[editing.id] !== true) {
        selectedIds = { ...selectedMap, [editing.id]: true }
      }
  ...
  /** Apply `mutate` to every selected text element and push the updated scene through updateScene. */
  function applyToSelected(mutate: (el: TextElementView) => TextElementView): void {
    const ids = new Set(selectedTextEls.map((e) => e.id))
    const next = snapshot.elements.map((el) => (ids.has(el.id) ? mutate(el) : el))
  ...
  function applyTypography(
    ...
    excalidrawAPI.updateScene({ elements: [...liveElements] as never })
    if (editingTextElement && typeof document !== 'undefined') {
      const editable = document.querySelector<HTMLTextAreaElement>('textarea.excalidraw-wysiwyg')
          ...
          <input
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === 'Escape') {
                ...
                fontSizeDraftRef.current = currentSize
                setFontSizeDraft(currentSize)
                restoreEditingSelectionOnEscape(event)
              }
            }}
          ...
          >
            {FONT_SIZES.map((size) => (
              <button
                ...
                onPointerDown={(event) => {
                  captureEditingSelection()
                  // Choosing a preset moves the pointer from the input into the portalled menu.
                  // Suppress that intermediate blur commit, otherwise it reapplies the old input
                  // value and closes the menu before this option's click can commit its size.
                  skipNextFontSizeBlurRef.current = true
                  event.preventDefault()
                }}
                onClick={() => {
        ...
        <ConfigProvider mountContainer={getUniverPortal()} locale={designLocale()}>
          ...
          >
            <ColorSplitControl
              ...
              initialColor={currentColor || TEXT_COLORS[0]}
              currentColor={currentColor || TEXT_COLORS[0]}
              renderIcon={(c) => (
                <FontColorDoubleIcon className="octo-tb-icon" extend={{ colorChannel1: c }} />
              )}
              onMainClick={(c) => applyColor(c)}
