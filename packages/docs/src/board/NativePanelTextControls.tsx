 ...
 * reaches Excalidraw history immediately through the vendor live-element mutation path; pending
 * creation defaults remain board-instance scoped until Excalidraw creates the next element.
 *
...
/**
 * Board's native renderer/editor contract currently supports only left/center/right. Imported
 * `customData.textAlign = 'justify'` is intentionally read as left until canvas, SVG/export and
 * WYSIWYG support land together; exposing or preserving a half-rendered justify state is unsafe.
 * TODO(board-justify): add justify here only after all three rendering/editing paths are proven.
 */
...
/** Coerce imported/default alignment to Board's currently supported native subset. */
...
export function NativePanelTextControls({ excalidrawAPI, initialSelection, targetIds, toolDefault = false, defaultsKey, runtime }: Props) {
  ...
  useEffect(() => {
    ...
    const updateSelection = (event: Event) => {
      ...
      const liveElement = (excalidrawAPI.getSceneElementsIncludingDeleted() as unknown as readonly TextElementView[])
        .find((element) => element.id === editingTextElement.id)
      ...
      const nextSelection: TextSelectionState = {
        ...
        // The editing element can be replaced without changing its id. Read the current scene
        // object rather than retaining customData from the effect render (or trusting an event that
        // was queued before the latest style write).
        textRuns: parseTextRuns(liveElement?.customData, value.length),
    ...
    const retainNativeSelection = (event: Event) => {
      ...
      const nextSelection: TextSelectionState = {
        ...
        textRuns: parseTextRuns(
          (excalidrawAPI.getSceneElementsIncludingDeleted() as unknown as readonly TextElementView[])
            .find((element) => element.id === editingTextElement.id)?.customData,
  ...
  const commitFontSize = (value: string) => {
    const liveApp = excalidrawAPI.getAppState() as { selectedElementIds?: Record<string, boolean>; editingTextElement?: { id?: string } | null }
    const liveOwner = liveApp.editingTextElement?.id || Object.keys(liveApp.selectedElementIds ?? {}).filter((id) => liveApp.selectedElementIds?.[id]).sort().join(',')
    if (!toolDefault && fontSizeOwnerRef.current && fontSizeOwnerRef.current !== liveOwner) {
      // Focus can move from the input to a newly selected canvas element before blur runs. Never
      // apply the old element's draft to that new owner (which would also flatten its text runs).
      fontSizeEditingRef.current = false
      fontSizeOwnerRef.current = ''
  ...
  const isUnambiguousPreset = (value: string): boolean =>
    FONT_SIZES.includes(value as typeof FONT_SIZES[number]) &&
    !FONT_SIZES.some((preset) => preset !== value && preset.startsWith(value))
  ...
  const applyAlign = (align: (typeof BOARD_TEXT_ALIGNS)[number]) => {
    if (toolDefault) {
      updateToolDefaults({ currentItemTextAlign: align })
      return
    }
    ...
    >
      ...
      <fieldset className="octo-board-native-inject__fs">
        ...
        <span
          ...
          onPointerDown={() => {
            captureEditingSelection()
            // A preset commit closes the menu but intentionally leaves this input focused so text
            // editing can continue. A second click therefore does not emit another focus event;
            // reopen explicitly from the trigger instead of requiring an outside click first.
            setFontFamilyOpen(false)
        ...
        >
          <input
            ...
            onFocus={() => {
              ...
              fontSizeOwnerRef.current = editingTextElement?.id || Object.keys(snapshot.selectedIds).filter((id) => snapshot.selectedIds[id]).sort().join(',')
              fontSizeDraftRef.current = currentSize
              setFontSizeDraft(currentSize)
              setFontSizeOpen(true)
            }}
