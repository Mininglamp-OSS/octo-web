...
/**
 * Rebase runs after one input edit, located by longest common UTF-16 prefix/suffix.
...
export function transformTextRuns(
  ...
  oldText: string,
  newText: string,
  insertedStyle?: BoardTextStyle,
): BoardTextRun[] {
  const normalized = normalizeTextRuns(runs, oldText.length)
  let prefix = 0
  const sharedLength = Math.min(oldText.length, newText.length)
  while (prefix < sharedLength && oldText.charCodeAt(prefix) === newText.charCodeAt(prefix)) prefix += 1
