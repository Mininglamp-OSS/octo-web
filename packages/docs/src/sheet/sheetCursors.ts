...
interface AwarenessLike {
  clientID: number
  getStates(): Map<number, { user?: { name?: string; color?: string }; cell?: { row: number; col: number } }>
...
export class SheetCursorOverlay {
  ...
  constructor(
    ...
    this.disposers.push(
      this.univerAPI.onCommandExecuted((cmd) => {
        if (cmd.id === SET_SELECTIONS_OP && this.interacted) this.broadcastSelection()
      }),
    )
  ...
  /**
   * Offset of the Univer grid canvas relative to our overlay mount. getCellRect is
   * relative to the grid canvas's origin, but the canvas sits BELOW Univer's toolbar +
   * formula bar inside the container — so we add this offset to land on the real cell.
   * The main grid canvas is the largest <canvas> under the (stable) mount.
   */
  ...
  private broadcastSelection(): void {
    ...
    // V1 is single-sheet; we key by row/col only (no per-client random sheetId, which
    // would never match across clients — same reason binding.ts uses a stable SHEET_KEY).
    this.awareness.setLocalStateField('cell', { row: r.startRow, col: r.startColumn })
  }
