...
- 全程不显示「加载失败」。
- 增补列表入口回归：详情接口延迟 1000ms，列表「编辑」仍在加载后打开并回填原正文；列表「重新生成」打开后切换全部重新生成，默认提示词保持原值，不依赖固定 300ms 时序。
- 增补失败 Agent 回归：保留旧正文，真实权限组合 `can_edit=false`、`can_edit_personal=true` 时仍可编辑并保存到 personal-edit，创建时间只展示一次。
