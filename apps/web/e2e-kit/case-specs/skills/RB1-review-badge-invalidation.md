...
- 打开待审核条目的详情，再打开拒绝原因弹窗，填写原因并提交。
- MSW 将拒绝请求挂起；按 Escape 时，两层弹窗均保持打开，详情取消按钮禁用。
- 测试显式释放请求并返回 409；拒绝弹窗保留输入及错误提示。
- 取消拒绝弹窗后，详情仍显示错误，供用户决定是否重试读取。

此场景覆盖原先自动 reload 清空错误、卸载拒绝弹窗导致失败反馈消失的问题。
