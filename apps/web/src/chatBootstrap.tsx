...
configureHtmlAttachmentRuntime(
  () => {
    ...
    return session &&
      session.uid === WKApp.loginInfo.uid &&
      session.token === WKApp.loginInfo.token
      ? session
      : null;
  },
  (listener) => {
    WKApp.mittBus.on("wk:auth-state-changed", listener);
    return () => WKApp.mittBus.off("wk:auth-state-changed", listener);
  }
);
