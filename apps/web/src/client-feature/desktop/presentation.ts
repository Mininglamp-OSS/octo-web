...
export interface DesktopPresentationBridge {
  getDesktopPresentation?: () => Promise<DesktopPresentation | null>;
  onDesktopPresentation?: (callback: (state: DesktopPresentation) => void) => () => void;
}

export interface DesktopReadyCapability<Page extends string = string> {
  desktopPresentationVersion?: 1;
  desktopPresentationPages?: readonly Page[];
}
...
const HEADERS = '[data-desktop-chrome="header"], [data-desktop-chrome="surface"]';
  ...
  const update = () => {
    ...
    for (const header of observed) {
      if (!headers.has(header)) {
        ...
        header.removeAttribute("data-desktop-surface");
    ...
    for (const header of headers) {
      ...
      if (inset) {
        const surface = header.dataset.desktopChrome === "surface";
        const attribute = surface ? "data-desktop-surface" : "data-desktop-header";
        header.removeAttribute(surface ? "data-desktop-header" : "data-desktop-surface");
        if (!header.hasAttribute(attribute)) header.setAttribute(attribute, "");
