...
function viteProxyResult(
  ...
  if (!isAgentMailboxAuthorization(authorization)) {
...
function forwardedHeaders(
  ...
  const stripped = new Set(strippedHeaders.map((header) => header.toLowerCase()));
  return Object.fromEntries(
    Object.entries(input).filter(([header]) => !stripped.has(header.toLowerCase()))
  );
}
...
describe("Agent Mail proxy path parity", () => {
  ...
  it("keeps the production Nginx location and rewrite directives aligned", () => {
    ...
    expect(nginx).toContain("location ~ ^/agent-mail-api(?:/|$)");
