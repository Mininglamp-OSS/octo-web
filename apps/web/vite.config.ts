...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        // HTML 文档后端(octo-docs-html)。前端默认把它的 base 解析成 /docs-html
        // (见 packages/docs/src/html/htmlDocFrameHelpers.ts:resolveOctoDocBase),没有这条代理
        // 时那些请求会落到 dev server 的 SPA 兜底页 —— 返回 200 + index.html,看着像通了,
        // 其实一个接口都没到后端。本地实测就是这么被骗过一次的。
