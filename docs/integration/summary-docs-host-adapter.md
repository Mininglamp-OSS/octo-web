...
> 对应 Client 侧 `docs-artifact-main-migration.md` 最后一节"总结转文档开发边界"的 Web 镜像。
...
- `isValidDocUrl` 校验结果 URL 的 origin 等于 `bootstrap.session.apiOrigin`，路径等于
  `/d/:docId`，无 query/hash/userinfo。
- `error.document` 链接同样经过安全校验后才暴露给用户。
- 转换 handler 在请求发起时捕获 spaceId/uid/token，结果回来时若任一字段变化则拒绝过期结果。
...
```bash
# docsPort 契约测试（含 getDocsDocumentOpener）
pnpm --filter @octo/base exec vitest run src/bridge/docs/docsPort.test.ts src/__tests__/docsPort.test.ts
