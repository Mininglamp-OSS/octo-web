...
- Direct creation publishes a minimal blank HTML v1 in the current Space with a 60-second client timeout. While publishing, closing and workflow switching are disabled; timeout becomes an uncertain terminal result and releases the loading lock.
...
- `registration_failed` is terminal and exposes its real slug for copying/recovery without inventing a doc id. Network failures, timeouts, malformed successful responses, and HTTP statuses without a clear non-publication contract are terminal because the client cannot safely know whether v1 was committed. Only the explicit non-publication allowlist (400/401/403/404/405/413/422/501) is retryable; 429 is uncertain.
- Bot creation fetches owned Bots only when entering Bot mode and retains a still-legal selection while switching modes, plus attachments, prompt preview, and the embedded private conversation. A `request_id` identifies the task, and remounts cannot auto-send it twice.
...
- Manual: in both zh-CN/en-US, exercise both workflows and equivalent preview states, direct success/open choices, Bot handoff, all close controls, retryable allowlisted HTTP failure, timeout/ambiguous failures, and registration slug recovery.
